...
def draw_crop_overlay(image: np.ndarray, crop_info: Dict[str, Any]) -> np.ndarray:
    ...
    if crop_info['crop_bounds']:
        ...
        # Top exclusion
        if min_y > 0:
            cv2.rectangle(excluded_overlay, (0, 0), (overlay.shape[1], min_y), (0, 0, 255), -1)
        
        # Bottom exclusion
        if max_y < overlay.shape[0]:
            cv2.rectangle(excluded_overlay, (0, max_y), (overlay.shape[1], overlay.shape[0]), (0, 0, 255), -1)
        
        # Left exclusion
        if min_x > 0:
            cv2.rectangle(excluded_overlay, (0, 0), (min_x, overlay.shape[0]), (0, 0, 255), -1)
        
        # Right exclusion
        if max_x < overlay.shape[1]:
            cv2.rectangle(excluded_overlay, (max_x, 0), (overlay.shape[1], overlay.shape[0]), (0, 0, 255), -1)
    ...
    # Draw text
    for i, line in enumerate(text_lines):
        color = (255, 255, 255) if i < 5 else ((0, 255, 0) if crop_info['has_crop_region'] else (0, 0, 255))
...
def create_spacing_analysis_plot(crop_info: Dict[str, Any]) -> np.ndarray:
    ...
    if not h_spacing and not v_spacing:
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(plot_img, "No spacing data available", (150, 200), font, 1.0, (0, 0, 0), 2)
        return plot_img
    ...
    if h_spacing:
        ...
        for i, spacing in enumerate(h_spacing):
            bar_height = int((spacing / max_h_spacing) * (plot_height - 100)) if max_h_spacing > 0 else 0
    ...
    if v_spacing:
        ...
        for i, spacing in enumerate(v_spacing):
            bar_height = int((spacing / max_v_spacing) * (plot_height - 100)) if max_v_spacing > 0 else 0
    ...
    if h_spacing:
        cv2.putText(plot_img, f"Avg: {crop_info['avg_h_spacing']:.1f}px", 
                   (10, plot_height - 20), font, 0.5, (0, 0, 255), 1)
    
    if v_spacing:
        cv2.putText(plot_img, f"Avg: {crop_info['avg_v_spacing']:.1f}px", 
                   (half_width + 10, plot_height - 20), font, 0.5, (255, 0, 0), 1)
    
    return plot_img


def create_crop_comparison(original: np.ndarray, overlay: np.ndarray,
                          cropped: np.ndarray, spacing_plot: np.ndarray,
                          crop_info: Dict[str, Any]) -> np.ndarray:
    ...
    if cropped is not None and cropped.size > 0:
        ...
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(cropped_resized, "No crop region", (50, target_height // 2), 
    ...
    cropped_label = create_label(f"Cropped Result ({crop_info['coverage_ratio']:.1%} coverage)", 
                                crop_width, crop_color)
...
def process_image_table_crop_visualization(image_path: Path, config: Config, 
    ...
    try:
        ...
        # Create comparison
        comparison = create_crop_comparison(image, overlay, cropped, spacing_plot, crop_info)
        ...
        if crop_info['has_crop_region']:
            print(f"  SUCCESS: Coverage: {crop_info['coverage_ratio']:.1%}, Size: {crop_info['crop_size']}")
...
def main():
    ...
    parser.add_argument("--crop-padding", type=int, default=10,
                       help="Padding around crop boundaries")
    ...
    if successful_results:
        crop_success_count = sum(1 for r in successful_results if r['crop_info']['has_crop_region'])
        print(f"Images with successful crops: {crop_success_count}/{len(successful_results)}")
        
        if crop_success_count > 0:
            avg_coverage = sum(r['crop_info']['coverage_ratio'] for r in successful_results if r['crop_info']['has_crop_region']) / crop_success_count
            print(f"Average coverage ratio: {avg_coverage:.1%}")
            
            coverage_range = [r['crop_info']['coverage_ratio'] for r in successful_results if r['crop_info']['has_crop_region']]
            print(f"Coverage range: {min(coverage_range):.1%} - {max(coverage_range):.1%}")
