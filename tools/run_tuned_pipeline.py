...
def create_tuned_stage1_config(input_dir, output_base, verbose=False, debug=False):
    ...
    return Stage1Config(
        ...
        roi_min_confidence_threshold=params['roi_detection']['roi_min_confidence_threshold'],
    )
...
def print_parameter_summary():
    ...
    print(f"   roi_min_confidence_threshold: {params['roi_detection']['roi_min_confidence_threshold']}")
    
    print("\n📏 Line Detection:")
    print(f"   Stage 1 - min_line_length: {params['line_detection']['stage1']['min_line_length']}px")
    print(f"   Stage 1 - max_line_gap: {params['line_detection']['stage1']['max_line_gap']}px")
    print(f"   Stage 2 - min_line_length: {params['line_detection']['stage2']['min_line_length']}px")
    print(f"   Stage 2 - max_line_gap: {params['line_detection']['stage2']['max_line_gap']}px")
    print()
...
Note: Update TUNED_PARAMETERS in this script with your optimal values first!
    ...
    parser.add_argument(
        "--show-params",
        action="store_true",
        help="Show parameter summary and exit"
    )
    ...
    try:
        ...
        if args.stage1_only:
            print(f"📁 Stage 1 results: {stage1_config.output_dir / '05_cropped_tables'}")
            print("💡 Next: Run with --stage2-only or run complete pipeline")
        elif args.stage2_only:
            print(f"📁 Stage 2 results: {stage2_config.output_dir / '04_fitted_tables'}")
        else:
