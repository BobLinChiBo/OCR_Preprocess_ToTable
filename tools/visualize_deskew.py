...
from output_manager import (get_default_output_manager, organize_visualization_output, get_test_images, 
                          convert_numpy_types, save_step_parameters)
...
def analyze_skew_detailed(image, angle_range=10, angle_step=0.2, 
                         min_angle_correction=0.5, save_intermediates=False, output_base=None):
    ...
    # Find optimal angle through exhaustive search (same as main pipeline)
    ...
    if save_intermediates and output_base:
        ...
        best_rotated = cv2.warpAffine(
            binary, 
            cv2.getRotationMatrix2D((binary.shape[1]//2, binary.shape[0]//2), best_angle, 1.0),
        ...
        proj_vis = create_projection_visualization(best_rotated, best_projection, best_angle)
...
def create_projection_visualization(binary_rotated: np.ndarray, projection: np.ndarray, angle: float) -> np.ndarray:
        ...
        if max_proj > 0:
            ...
            for i, val in enumerate(normalized_proj):
                if i < height:
                    bar_length = int(val)
                    if bar_length > 0:
                        cv2.line(vis_img, (plot_start_x, i), (plot_start_x + bar_length, i), (0, 255, 0), 1)
    
    # Add labels and info
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(vis_img, f"Angle: {angle:.2f}°", (plot_start_x, 30), font, 0.6, (255, 255, 255), 1)
    cv2.putText(vis_img, "Horizontal Projection", (plot_start_x, 50), font, 0.5, (255, 255, 255), 1)
    cv2.putText(vis_img, "(Row pixel sums)", (plot_start_x, 65), font, 0.4, (128, 128, 128), 1)
    
    return vis_img


def create_angle_score_plot(angles: np.ndarray, scores: List[float], best_angle: float) -> np.ndarray:
    ...
    # Draw axes
    cv2.line(plot_img, (margin, plot_height - margin), (plot_width - margin, plot_height - margin), (0, 0, 0), 2)  # x-axis
    cv2.line(plot_img, (margin, margin), (margin, plot_height - margin), (0, 0, 0), 2)  # y-axis
    
    # Plot score curve
    points = []
    for i, (angle, score) in enumerate(zip(angles, scores)):
        x = margin + int((i / (len(angles) - 1)) * plot_area_width)
        y = plot_height - margin - int(((score - min_score) / score_range) * plot_area_height)
        points.append((x, y))
    ...
    # Add labels
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(plot_img, "Histogram Variance Scores", (margin, 30), font, 0.7, (0, 0, 0), 2)
    cv2.putText(plot_img, f"Best: {best_angle:.2f}° (score: {max_score:.0f})", (margin, plot_height - 10), font, 0.5, (0, 128, 0), 1)
    
    # Angle labels on x-axis
    for i in range(0, len(angles), max(1, len(angles) // 5)):
        x = margin + int((i / (len(angles) - 1)) * plot_area_width)
        cv2.putText(plot_img, f"{angles[i]:.1f}°", (x - 15, plot_height - 25), font, 0.4, (0, 0, 0), 1)
...
def draw_line_detection_overlay(image: np.ndarray, skew_info: Dict[str, Any]) -> np.ndarray:
    ...
    cv2.putText(overlay, "Target alignment", (ref_x1, ref_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    ...
    if abs(rotation_angle) > 0.1:
        cv2.ellipse(overlay, (center_x, center_y), (60, 60), 0, 0, -rotation_angle, (255, 255, 0), 2)
...
def create_angle_histogram_plot(skew_info: Dict[str, Any]) -> np.ndarray:
    ...
    if not skew_info.get('scores'):
        ...
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(plot_img, "No scores available", (200, 150), font, 1.0, (0, 0, 0), 2)
        return plot_img
    ...
    for i, (angle, score) in enumerate(zip(angles, scores)):
        bar_height = int(((score - min_score) / score_range) * (plot_height - 80)) if score_range > 0 else 0
        x = i * bar_width
        y = plot_height - 40 - bar_height
        
        # Color based on score value (higher scores are greener)
        score_intensity = int(((score - min_score) / score_range) * 255) if score_range > 0 else 0
    ...
    # Add labels
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(plot_img, "Histogram Variance Scores", (10, 25), font, 0.7, (0, 0, 0), 2)
    cv2.putText(plot_img, f"Best: {rotation_angle:.2f}° (score: {max_score:.0f})", (10, plot_height - 10), font, 0.6, (0, 255, 0), 1)
...
def create_deskew_comparison(original: np.ndarray, deskewed: np.ndarray, 
                           ...
                           binary_vis: np.ndarray, skew_info: Dict[str, Any]) -> np.ndarray:
...
def process_image_deskew_visualization(image_path: Path, config, 
                                     output_dir: Path, use_organized_output: bool = True,
    ...
    try:
        ...
        output_base = str(output_dir / base_name) if use_organized_output else str(output_dir / base_name)
        save_intermediates = command_args.get('save_intermediates', False) if command_args else False
        skew_info = analyze_skew_detailed(image, config.angle_range, config.angle_step, 
                                        config.min_angle_correction,
                                        save_intermediates=save_intermediates, output_base=output_base)
        ...
        # Create comparison
        comparison = create_deskew_comparison(image, deskewed, overlay, angle_plot, binary_vis, skew_info)
        ...
        try:
            param_file = save_step_parameters(
                ...
                output_dir=output_dir if not use_organized_output else output_dir.parent,
...
def main():
    ...
    if args.config_file and args.config_file.exists():
        config_source = "file"
    if any(v is not None for v in [args.angle_range, args.angle_step, args.min_angle_correction]):
    ...
    if use_organized:
        print(f"Use 'python tools/check_results.py latest deskew --view' to view results")
        print(f"Use 'python tools/check_results.py list' to see all runs")
    else:
        print(f"Review the '_08_deskew_comparison.jpg' files to assess deskewing quality")
