...
class OutputManager:
    ...
    def get_latest_run(self, script_name: str) -> Optional[Path]:
        ...
        run_dirs = sorted(
            [d for d in self.base_dir.glob(pattern) if d.is_dir()], reverse=True
        )
    ...
    def list_runs(self, script_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for run_dir in run_dirs:
            ...
            if len(parts) >= 3:
                ...
                # Count output files
                image_count = (
                    len(list((run_dir / "images").glob("*.jpg")))
                    if (run_dir / "images").exists()
                    else 0
                )
                analysis_count = (
                    len(list((run_dir / "analysis").glob("*.json")))
...
def organize_visualization_output(
    ...
    metadata = {
        ...
        "has_parameter_documentation": parameter_file is not None
        or "parameters" in organized_files,
    }
...
def create_html_viewer(run_dir: Path) -> Path:
    ...
    if "analysis_summary" in metadata:
        ...
        """.format(
            json.dumps(metadata["analysis_summary"], indent=2)
        )
...
def save_step_parameters(
    ...
    param_docs = {}
    if hasattr(config_obj, "__class__") and hasattr(
        config_obj.__class__, "__dataclass_fields__"
    ):
...
def extract_success_indicators(processing_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "roi_info" in processing_results:
        ...
        if roi:
            roi_area = (roi.get("roi_right", 0) - roi.get("roi_left", 0)) * (
                roi.get("roi_bottom", 0) - roi.get("roi_top", 0)
            )
...
def create_parameter_comparison(param_files: List[Path], output_dir: Path) -> Path:
    ...
    for step, items in by_step.items():
        ...
        for item in items:
            indicators = (
                item["data"].get("processing_results", {}).get("success_indicators", {})
            )
...
def validate_parameter_usage(param_file: Path) -> Dict[str, Any]:
    ...
    if step_name == "deskew":
        ...
        if indicators.get("deskew_confidence", 0) < 0.5:
            validation["parameter_warnings"].append(
                "Low deskew confidence - consider adjusting angle_range or angle_step"
            )

        if indicators.get("line_count", 0) < 10:
            validation["parameter_warnings"].append(
                "Few lines detected - image may not be suitable for skew detection"
            )
        ...
        if coverage < 0.1:
            validation["parameter_warnings"].append(
                "Very small ROI detected - parameters may be too aggressive"
            )
        elif coverage > 0.95:
            validation["parameter_warnings"].append(
                "ROI covers almost entire image - parameters may be too conservative"
            )

        validation["effectiveness_score"] = (
            min(1.0, coverage * 2) if coverage < 0.5 else (2 - coverage * 2)
        )
        ...
        if h_lines == 0 or v_lines == 0:
            validation["parameter_warnings"].append(
                "Missing horizontal or vertical lines - adjust line detection parameters"
            )
    ...
    if len(overrides) > 3:
        validation["suggestions"].append(
            "Many parameter overrides used - consider updating default config"
        )
...
if __name__ == "__main__":
    ...
    if runs:
        ...
        for run in runs[:10]:  # Show latest 10
            print(
                f"  {run['script']} - {run['timestamp'].strftime('%Y-%m-%d %H:%M')} - {run['image_count']} images"
            )
    else:
