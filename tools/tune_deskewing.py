...
def test_deskewing_parameters():
    ...
    for i, (angle_range, angle_step, min_angle_correction) in enumerate(param_combinations, 1):
        ...
        with open(angle_log_file, 'w') as f:
            ...
            for entry in angle_log:
                ...
                f.write(f"{entry['image']}: {entry['detected_angle']:.2f}° ({status})\n")
            
            # Statistics
            corrected_count = sum(1 for entry in angle_log if entry['correction_applied'])
    ...
    with open(summary_file, 'w') as f:
        ...
        for result in results_summary:
            corrected = sum(1 for entry in result['angle_log'] if entry['correction_applied'])
            avg_angle = np.mean([abs(entry['detected_angle']) for entry in result['angle_log']])
