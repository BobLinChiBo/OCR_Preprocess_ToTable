...
def test_line_detection_parameters():
    ...
    for i, (min_line_length, max_line_gap) in enumerate(param_combinations, 1):
        ...
        with open(analysis_file, 'w') as f:
            ...
            if line_log:
                ...
                avg_h_lines = sum(entry['h_lines'] for entry in line_log) / len(line_log)
                avg_v_lines = sum(entry['v_lines'] for entry in line_log) / len(line_log)
                avg_crop_ratio = sum(entry['crop_area_ratio'] for entry in line_log) / len(line_log)
    ...
    with open(summary_file, 'w') as f:
        ...
        for result in results_summary:
            if result['line_log']:
                detected = sum(1 for entry in result['line_log'] if entry['lines_detected'])
                detection_rate = detected / len(result['line_log'])
                avg_h = sum(entry['h_lines'] for entry in result['line_log']) / len(result['line_log'])
                avg_v = sum(entry['v_lines'] for entry in result['line_log']) / len(result['line_log'])
                avg_crop = sum(entry['crop_area_ratio'] for entry in result['line_log']) / len(result['line_log'])
            else:
                detection_rate = 0
        ...
        f.write("-" * 30 + "\n")
        f.write("• Lower min_line_length: More sensitive, detects shorter line segments\n")
