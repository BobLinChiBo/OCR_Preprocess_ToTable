...
def guide_file_copying(source_stage, target_dir, file_pattern="*.jpg"):
    ...
    if sys.platform.startswith("win"):
        print(
            f'copy "data\\output\\tuning\\{source_stage}\\best_folder\\{file_pattern}" "{target_dir}\\"'
        )
    else:
        print(
            f"cp data/output/tuning/{source_stage}/best_folder/{file_pattern} {target_dir}/"
        )
    ...
    while True:
        response = prompt_user(
            "Have you copied the best results to the target directory? (y/n)",
            ["y", "n"],
        )
        ...
        if response == "y":
            ...
            if check_directory_has_files(target_dir, file_pattern):
                ...
                print(
                    "❌ No files found in target directory. Please check your copy command."
                )
                continue
        else:
...
def main():
    ...
    if not state.is_completed("page_splitting"):
        ...
        success = run_script(
            "tools/tune_page_splitting.py", "Page splitting parameter tuning"
        )
        if success:
            show_evaluation_tips("page_splitting")

            if guide_file_copying(
                "01_split_pages", "data/output/tuning/02_deskewed_input"
            ):
    ...
    if not state.is_completed("roi_detection"):
        ...
        success = run_script(
            "tools/tune_roi_detection.py", "ROI detection parameter tuning"
        )
        if success:
            show_evaluation_tips("roi_detection")

            if guide_file_copying(
                "03_roi_detection", "data/output/tuning/04_line_input", "*_roi.jpg"
            ):
    ...
    if not state.is_completed("line_detection"):
        ...
        success = run_script(
            "tools/tune_line_detection.py", "Line detection parameter tuning"
        )
        if success:
            ...
            updated = prompt_user(
                "Have you updated the parameters in run_tuned_pipeline.py? (y/n)",
                ["y", "n"],
            )
            if updated == "y":
                ...
                print(
                    "❌ Please update the parameters before proceeding to the final stage."
                )
                return
        else:
    ...
    if not state.is_completed("final_pipeline"):
        ...
        success = run_script(
            "tools/run_tuned_pipeline.py data/input/test_images/ --verbose",
