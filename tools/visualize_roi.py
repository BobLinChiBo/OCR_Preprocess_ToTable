...
def draw_roi_overlay(image: np.ndarray, roi_coords: Dict[str, Any], 
                    show_gabor: bool = False, config = None, analysis: Dict = None) -> np.ndarray:
    ...
    if analysis:
        ...
        cv2.rectangle(roi_overlay, (right_search_start, 0), (width, height), (0, 255, 255), -1)
        ...
        cv2.rectangle(roi_overlay, (0, bottom_search_start), (width, height), (255, 0, 255), -1)
    ...
    overlay = cv2.addWeighted(overlay, 1-alpha_excluded, excluded_overlay, alpha_excluded, 0)
    ...
    if analysis and 'vertical_info' in analysis:
        ...
        left_color = (0, 255, 0) if left_applied else (0, 100, 255)  # Green if applied, orange if not
        if left > 0:
            ...
            if 'left_cut_strength' in vertical_info:
                ...
                cv2.putText(overlay, f"L:{strength:.1f}", (left + 5, 25), font, 0.5, left_color, 2)
        ...
        if right < width:
            ...
            if 'right_cut_strength' in vertical_info:
                ...
                cv2.putText(overlay, f"R:{strength:.1f}", (right - 60, 25), font, 0.5, right_color, 2)
    ...
    if analysis and 'horizontal_info' in analysis:
        ...
        if top > 0:
            ...
            if 'top_cut_strength' in horizontal_info:
                ...
                cv2.putText(overlay, f"T:{strength:.1f}", (5, top - 5), font, 0.5, top_color, 2)
        ...
        if bottom < height:
            ...
            if 'bottom_cut_strength' in horizontal_info:
                ...
                cv2.putText(overlay, f"B:{strength:.1f}", (5, bottom + 20), font, 0.5, bottom_color, 2)
    ...
    for i, line in enumerate(text_lines):
        ...
        cv2.rectangle(overlay, (10, text_y_start + i*30 - 20), 
                     (20 + text_size[0], text_y_start + i*30 + 8), 
                     (0, 0, 0), -1)
    
    # Draw text
    for i, line in enumerate(text_lines):
        cv2.putText(overlay, line, (15, text_y_start + i*30), 
    ...
    if show_gabor and config and analysis and 'binary_mask' in analysis:
        ...
        cv2.putText(overlay, f"{method_name} Response", (x_offset, y_offset-5), 
...
def create_comparison_grid(original: np.ndarray, roi_overlay: np.ndarray, 
                          cropped_roi: np.ndarray = None, analysis: Dict = None, 
                          roi_coords: Dict = None, show_projections: bool = True) -> np.ndarray:
    ...
    if show_projections and analysis and roi_coords:
        ...
        vertical_plot = create_vertical_projection_plot(vertical_info, original.shape[1], roi_coords)
        horizontal_plot = create_horizontal_projection_plot(horizontal_info, original.shape[0], roi_coords)
        ...
        if cropped_roi is not None and cropped_roi.size > 0:
            ...
            h_proj_label = create_label("Horizontal Projection", crop_width, (255, 0, 255))
            
            # Combine cropped ROI with horizontal projection
            right_panel = np.vstack([
                crop_label, cropped_resized,
                h_proj_label, h_plot_resized
            ])
            
            # Pad to match left panel height if needed
            height_diff = left_panel.shape[0] - right_panel.shape[0]
            if height_diff > 0:
                padding = np.zeros((height_diff, right_panel.shape[1], 3), dtype=np.uint8)
                right_panel = np.vstack([right_panel, padding])
            elif height_diff < 0:
                padding = np.zeros((-height_diff, left_panel.shape[1], 3), dtype=np.uint8)
            ...
            h_proj_label = create_label("Horizontal Projection", h_plot_width, (255, 0, 255))
        ...
        if vertical_info and horizontal_info:
            ...
            return create_comparison_grid(original, roi_overlay, cropped_roi, analysis, roi_coords, True)
...
def save_gabor_debug_images(gray_img: np.ndarray, config, output_dir: Path, base_name: str):
...
def save_canny_sobel_debug_images(gray_img: np.ndarray, config, output_dir: Path, base_name: str):
    """Save debug images for Canny + Sobel method."""
    # Step 2: Apply Gaussian blur
    blurred = cv2.GaussianBlur(gray_img, (config.gaussian_blur_size, config.gaussian_blur_size), 0)
    cv2.imwrite(str(output_dir / f"{base_name}_02_blurred.jpg"), blurred)
    
    # Step 3: Canny edge detection
    canny_edges = cv2.Canny(blurred, config.canny_low_threshold, config.canny_high_threshold)
    ...
    sobel_x_norm = cv2.normalize(np.abs(sobel_x), None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    sobel_y_norm = cv2.normalize(np.abs(sobel_y), None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    ...
    cv2.imwrite(str(output_dir / f"{base_name}_05_sobel_magnitude.jpg"), sobel_magnitude)
    ...
    combined_edges = cv2.normalize(combined_edges, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    ...
    _, binary_mask = cv2.threshold(morph_final, config.edge_binary_threshold, 255, cv2.THRESH_BINARY)
...
def save_adaptive_debug_images(gray_img: np.ndarray, config, output_dir: Path, base_name: str):
    """Save debug images for Adaptive threshold method."""
    # Step 2: Apply adaptive thresholding
    adaptive_method = (
        cv2.ADAPTIVE_THRESH_MEAN_C if config.adaptive_method == 'mean'
        else cv2.ADAPTIVE_THRESH_GAUSSIAN_C
    )
    adaptive_thresh = cv2.adaptiveThreshold(
        gray_img, 255, adaptive_method, cv2.THRESH_BINARY, config.adaptive_block_size, config.adaptive_C
    )
    cv2.imwrite(str(output_dir / f"{base_name}_02_adaptive_thresh.jpg"), adaptive_thresh)
    ...
    if config.edge_enhancement:
        ...
        grad_x_norm = cv2.normalize(np.abs(grad_x), None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        grad_y_norm = cv2.normalize(np.abs(grad_y), None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        ...
        cv2.imwrite(str(output_dir / f"{base_name}_04_gradient_magnitude.jpg"), gradient_magnitude)
        
        # Step 5: Threshold gradient
        _, gradient_mask = cv2.threshold(gradient_magnitude, 50, 255, cv2.THRESH_BINARY)
        cv2.imwrite(str(output_dir / f"{base_name}_05_gradient_mask.jpg"), gradient_mask)
        
        # Step 6: Combine adaptive threshold with gradient edges
        combined_mask = cv2.bitwise_or(adaptive_thresh, gradient_mask)
        cv2.imwrite(str(output_dir / f"{base_name}_06_combined_mask.jpg"), combined_mask)
    else:
    ...
    if config.edge_enhancement:
        debug_files.update({
            ...
            'gradient_magnitude': str(output_dir / f"{base_name}_04_gradient_magnitude.jpg"),
...
def create_vertical_projection_plot(vertical_info: Dict, width: int, roi_coords: Dict) -> np.ndarray:
    ...
    if max_val > min_val:
        normalized = ((projection - min_val) / (max_val - min_val) * (plot_height - 60)).astype(int)
    else:
    ...
    cv2.rectangle(plot_img, (right_search_start, 0), (plot_width, plot_height), (200, 255, 200), -1)
    ...
    if roi_left > 0:
        ...
        cv2.putText(plot_img, f"L:{roi_left}", (roi_left + 5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, left_color, 1)
    ...
    if roi_right < width:
        ...
        cv2.putText(plot_img, f"R:{roi_right}", (roi_right - 40, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, right_color, 1)
    
    # Add labels and statistics
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(plot_img, "Vertical Projection", (10, 15), font, 0.6, (0, 0, 0), 1)
    cv2.putText(plot_img, f"Min: {min_val:.0f}", (10, plot_height - 5), font, 0.4, (0, 0, 0), 1)
    cv2.putText(plot_img, f"Max: {max_val:.0f}", (plot_width - 80, plot_height - 5), font, 0.4, (0, 0, 0), 1)
    
    # Add cut strength info if available
    if 'left_cut_strength' in vertical_info:
        left_strength = vertical_info['left_cut_strength']
        cv2.putText(plot_img, f"L_str: {left_strength:.1f}", (10, plot_height - 20), font, 0.4, (0, 0, 0), 1)
    if 'right_cut_strength' in vertical_info:
        right_strength = vertical_info['right_cut_strength']
        cv2.putText(plot_img, f"R_str: {right_strength:.1f}", (plot_width - 120, plot_height - 20), font, 0.4, (0, 0, 0), 1)
    
    return plot_img


def create_horizontal_projection_plot(horizontal_info: Dict, height: int, roi_coords: Dict) -> np.ndarray:
    ...
    min_val = np.min(projection) if len(projection) > 0 else 0
    if max_val > min_val:
        normalized = ((projection - min_val) / (max_val - min_val) * (plot_width - 60)).astype(int)
    else:
    ...
    cv2.rectangle(plot_img, (0, bottom_search_start), (plot_width, plot_height), (200, 255, 200), -1)
    ...
    if roi_top > 0:
        ...
        cv2.putText(plot_img, f"T:{roi_top}", (5, roi_top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, top_color, 1)
    ...
    if roi_bottom < height:
        ...
        cv2.putText(plot_img, f"B:{roi_bottom}", (5, roi_bottom + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, bottom_color, 1)
    ...
    cv2.putText(plot_img, f"Min: {min_val:.0f}", (5, plot_height - 25), font, 0.4, (0, 0, 0), 1)
    cv2.putText(plot_img, f"Max: {max_val:.0f}", (5, plot_height - 10), font, 0.4, (0, 0, 0), 1)
    
    # Add cut strength info if available
    if 'top_cut_strength' in horizontal_info:
        top_strength = horizontal_info['top_cut_strength']
        cv2.putText(plot_img, f"T_str: {top_strength:.1f}", (120, 20), font, 0.4, (0, 0, 0), 1)
    if 'bottom_cut_strength' in horizontal_info:
        bottom_strength = horizontal_info['bottom_cut_strength']
        cv2.putText(plot_img, f"B_str: {bottom_strength:.1f}", (120, plot_height - 10), font, 0.4, (0, 0, 0), 1)
    
    return plot_img


def process_image_visualization(image_path: Path, config, 
                              output_dir: Path, show_gabor: bool = False, save_debug: bool = False, 
                              show_projections: bool = True, command_args: Dict[str, Any] = None,
    ...
    try:
        ...
        roi_coords, analysis = ocr_utils.detect_roi_for_image(image, config, return_analysis=True)
        ...
        comparison = create_comparison_grid(image, roi_overlay, cropped_roi, analysis, roi_coords, show_projections)
        ...
        if vertical_info:
            vertical_plot = create_vertical_projection_plot(vertical_info, image.shape[1], roi_coords)
        ...
        if horizontal_info:
            horizontal_plot = create_horizontal_projection_plot(horizontal_info, image.shape[0], roi_coords)
...
def main():
    ...
    parser.add_argument("--show-gabor", action="store_true",
                       help="Show edge detection response overlay")
    parser.add_argument("--save-debug", action="store_true",
                       help="Save debug images showing binarization and edge detection steps")
    parser.add_argument("--show-projections", action="store_true", default=True,
                       help="Show projection graphs and cutting lines analysis (enabled by default)")
    parser.add_argument("--hide-projections", action="store_true",
    ...
    parser.add_argument("--roi-method", type=str, 
    ...
    parser.add_argument("--adaptive-block-size", type=int, default=None,
    ...
    if args.config_file and args.config_file.exists():
        config_source = "file"
    if args.config_params or any(v is not None for v in [args.roi_method, args.gabor_threshold, args.cut_strength, 
                                                         args.confidence_threshold, args.canny_low, args.canny_high, 
    ...
    for i, image_path in enumerate(image_paths, 1):
        ...
        result = process_image_visualization(
            ...
            args.show_projections, command_args, config_source
        )
