...
class ROIConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.roi_min_confidence_threshold = kwargs.get(
            "roi_min_confidence_threshold", 5.0
        )
...
def test_roi_detection_parameters():
    ...
    estimated_runtime_min = (
        total_combinations * len(image_files) * 0.5
    ) / 60  # ~0.5 sec per image

    print(f"Testing {total_combinations} parameter combinations:")
    print("Parameters: kernel_size, sigma, lambda, cut_strength, confidence")
    print(
        f"Estimated runtime: {estimated_runtime_min:.1f} minutes ({total_combinations * len(image_files)} image processes)"
    )
    ...
    try:
        for i, (kernel_size, sigma, lambda_val, cut_strength, confidence) in enumerate(
            param_combinations, 1
        ):
            param_name = (
                f"k{kernel_size}_s{sigma}_l{lambda_val}_cs{cut_strength}_ct{confidence}"
            )
            ...
            for image_path in image_files:
                try:
                    ...
                    roi_coords, analysis = detect_roi_for_image(
                    ...
                    analysis_stats = {
                        ...
                        "original_size": (
                            roi_coords["image_width"],
                            roi_coords["image_height"],
                        ),
                    }
                    ...
                    print(
                        f"    [OK] {image_path.name} -> area: {analysis_stats['area_ratio']:.2%}"
                    )
            ...
            if roi_log:
                avg_area_ratio = sum(entry["area_ratio"] for entry in roi_log) / len(
                    roi_log
                )
                in_target_count = sum(
                ...
                print(
                    f"    [RESULT] Average area: {avg_area_ratio:.2%}, Target range (65-95%): {in_target_count}/{len(roi_log)} ({success_rate:.1f}%)"
                )
                ...
                if success_rate >= 80:
                    ...
                    print(
                        f"    [SUCCESS] *** PROMISING COMBINATION - {success_rate:.1f}% success rate ***"
                    )
            ...
            with open(analysis_file, "w") as f:
                ...
                for entry in roi_log:
                    ...
                    f.write(
                        f"  Size: {entry['cropped_size'][0]}x{entry['cropped_size'][1]}\n"
                    )
                ...
                if roi_log:
                    avg_area_ratio = sum(
                        entry["area_ratio"] for entry in roi_log
                    ) / len(roi_log)
                    in_target_count = sum(
                        1 for entry in roi_log if 0.65 <= entry["area_ratio"] <= 0.95
                    )
                    ...
                    f.write(
                        f"  Target range (65-95%): {in_target_count}/{len(roi_log)} ({success_rate:.1f}%)\n"
                    )
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(
            f"Parameter combinations tested: {len(results_summary)}/{total_combinations}\n"
        )
        f.write(
            f"Successful combinations (80%+ in 65-95% range): {len(successful_combinations)}\n\n"
        )
        ...
        if successful_combinations:
            ...
            successful_combinations.sort(
                key=lambda x: (x["success_rate"], -abs(x["avg_area_ratio"] - 0.80)),
                reverse=True,
            )
        ...
        results_summary.sort(
            key=lambda x: (
                sum(entry["area_ratio"] for entry in x["roi_log"]) / len(x["roi_log"])
                if x["roi_log"]
                else 0
            ),
            reverse=True,
        ...
        for result in results_summary:
            if result["roi_log"]:
                avg_area = sum(
                    entry["area_ratio"] for entry in result["roi_log"]
                ) / len(result["roi_log"])
                in_target_count = sum(
                    1
                    for entry in result["roi_log"]
                    if 0.65 <= entry["area_ratio"] <= 0.95
                )
            ...
            status = (
                ...
                if success_rate >= 80
                    ...
                    "★★" if success_rate >= 50 else "★" if success_rate >= 20 else "   "
                )
            )
            ...
            f.write(
                f"  Target range success: {success_rate:.1f}% ({in_target_count}/{len(result['roi_log']) if result['roi_log'] else 0})\n"
            )
    ...
    print(
        f"[TARGET] {len(successful_combinations)} combinations achieved 65-95% target range"
    )
    ...
    if successful_combinations:
        ...
        for i, success in enumerate(successful_combinations[:3], 1):  # Top 3
            print(
                f"  {i}. {success['params']} - {success['success_rate']:.1f}% success, {success['avg_area_ratio']:.1%} avg area"
            )
    else:
