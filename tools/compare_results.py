...
def create_comparison_grid(image_paths, labels, output_path, grid_cols=3):
    ...
    for i, img in enumerate(resized_images):
        ...
        if width < max_width:
            ...
            padded = cv2.copyMakeBorder(img, 0, 0, 0, padding, cv2.BORDER_CONSTANT, value=[255, 255, 255])
        else:
            padded = img
        ...
        cv2.putText(label_img, labels[i] if i < len(labels) else f"Image {i+1}", 
...
def compare_page_splitting(top_n=None, specific_image=None):
    ...
    param_folders = [d for d in results_dir.iterdir() if d.is_dir() and not d.name.startswith('.')]
    ...
    for base_name in test_images:
        ...
        for page_num in [1, 2]:
            ...
            if page_images:
                output_path = comparison_dir / f"{base_name}_page_{page_num}_comparison.jpg"
...
def generate_summary_report():
    ...
    with open(report_path, 'w') as f:
        ...
        if split_dir.exists():
            ...
            f.write("- Key evaluation criteria: Clean gutter separation, no content loss\n\n")
        ...
        if deskew_dir.exists():
            ...
            f.write("- Key evaluation criteria: Horizontal text, straight table borders\n\n")
        ...
        if roi_dir.exists():
            ...
            f.write("- Key evaluation criteria: Content focus, margin removal, preservation\n\n")
        ...
        if line_dir.exists():
            ...
            f.write("- Key evaluation criteria: Table line detection, noise filtering\n\n")
...
Examples:
  ...
  python tools/compare_results.py summary
    ...
    parser.add_argument(
        "stage",
        choices=["page_splitting", "deskewing", "roi_detection", "line_detection", "summary"],
    ...
    parser.add_argument(
        "--top",
        type=int,
        help="Compare only top N parameter combinations"
    )
    
    parser.add_argument(
        "--image",
        help="Compare only specific image (partial name match)"
    )
