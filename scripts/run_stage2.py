...
Examples:
  ...
  python run_stage2.py cropped/ -o refined/ --verbose       # Custom input/output with verbose
    ...
    if not input_path.is_dir():
        print(f"Error: Stage 2 requires a directory of cropped table images: {input_path}")
        sys.exit(1)
    ...
    try:
        if args.verbose:
            ...
            print(f"   - Debug images: {'enabled' if stage2_config.save_debug_images else 'disabled'}")
            print()
        ...
        if args.verbose:
            ...
            print("   Your tables are ready for use in publications, reports, or further analysis.")
        else:
